...
local function start(self, ...)
    ...
    local func, opts = ...
    -- Cancel every worker in the pool except a failed one. A
    -- failed fiber worker kills the rest of the pool from its
    -- own fiber, releasing workers that wait on synchronization
    -- primitives. Coroutine-based threads are not cancelled
    -- here: a failed worker makes the scheduler drop the
    ...
    -- Wrap a worker function to implement a fail-stop behaviour:
    -- when a worker fails, the rest of the pool is cancelled and
    -- the first failure is reported to a caller.
    local function fiber_worker(thread_id)
        ...
        if res[2] == false then
            local err = res[3]
            self.failure = self.failure ~= nil and self.failure or
                (err ~= nil and tostring(err) or 'worker returned an error')
            abort_workers(thread_id)
        end
        return unpack(res, 2)
    end
    ...
    for thread_id = 1, self.thread_num do
        ...
        local thread_func = self.thread_type == 'fiber' and fiber_worker or func
