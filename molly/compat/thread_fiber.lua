...
--
-- Fibers are a unique Tarantool feature - 'green' threads or
-- coroutines that run independently of operating system threads.
-- Fibers nicely illustrate Tarantool's theoretical grounding in
-- the actor model, which is based on the concept of a number of
-- light processes that cooperatively multitask and communicate
-- with one another through messaging. An advantageous feature of
-- Tarantool fibers is that they include local storage.
--
-- Tarantool is programmed using Lua, which has its own
-- coroutines, but due to the way that fibers interface with
-- Tarantool's asynchronous event loop, it is better to use fibers
-- rather than Lua coroutines. Fibers work well with all of the
...
local function create(self, ...)
    ...
    if fiber_obj:status() ~= 'dead' then
        ...
        -- Needed for backward compatibility with 1.7.
        fiber_obj:wakeup()
